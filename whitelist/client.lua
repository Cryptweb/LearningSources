-- [0xAAF] - Couldn't find a request function that works with the whitelist

--// Define our request function
local http_request = request or http_request;
--// Define the client/local player
local client = game:GetService("Players").LocalPlayer

--// If user is using synapse
if (syn) then
    http_request = syn.request; --// Set http_request to synapse's built-in request function
end;

--// If their executor doesn't have a request function that works with the whitelist
if (not http_request) then
    return client:Kick('Error code [0xAAF]') --// Kick them
end;

--// Define our domain
local domain = 'http://localhost:8080/wl/';
...
--// Secret keys
local secret1 = 'GYW6dk8{Z5rnzDPHSaR-fB7E'; --// Make sure it is atleast 24 characters long and is hard to rewrite.
local secret2 = 'GzsTENr2m9PU-XiBU5oX3Dgj'; --// Same as above

--// Send our HWID to the server
http_request({Url = domain .. 'getHWID', Method = 'GET'})
--// Get our response
...
local expected = hmac('success' .. secret2, secret1, secret1 .. 'whitelisted');
--// Check if the response equals to our expected response
